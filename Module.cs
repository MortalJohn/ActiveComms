    ...
    public class Module : Blish_HUD.Modules.Module {
        ...
        private void ExtractAndLoadWhisperNatives() {
            ...
            try {
                ...
                Logger.Warn(ex, "File access denied extracting Whisper natives — security software may be blocking writes to {0}.", runtimeDir);
                Contingency.NotifyFileSaveAccessDenied(runtimeDir, "extract Whisper AI audio processing libraries");
                return;
            }
        ...
        protected override void OnModuleLoaded(EventArgs e) {
            ...
            _cornerIcon = new CornerIcon {
                ...
                IconName  = "Active Comms: Initialising\u2026"
            };
        ...
        private void PopulateMicrophoneDevice() {
            ...
            if (devices.Length == 0) {
                Logger.Warn("ActiveComms: No microphone devices found.");
                return;
            }

            // Log all available devices so users can find the exact name.
            Logger.Info($"ActiveComms: Available microphone devices: {string.Join(", ", devices)}");
            ...
            if (devices.Length == 1 && string.IsNullOrWhiteSpace(_microphoneDevice.Value)) {
                ...
                Logger.Info($"ActiveComms: Auto-selected microphone: {devices[0]}");
            }
        }
        ...
        public override IView GetSettingsView() {
            ...
            var btn = new StandardButton {
                Parent = panel,
                Text   = "Open Active Comms Settings\u2026",
                Width  = 240,
                Height = 26,
                Top    = 4,
            };
            ...
            try {
                ...
                if (_cornerIcon != null) {
                    ...
                    _cornerIcon.IconName = hasMic
                        ? "Active Comms: Ready"
                        : "Active Comms: No microphone detected";
                }
            } catch (OperationCanceledException) {
                // Settings changed or module unloading — silently exit.
            } catch (Exception ex) when (ex.HResult == -2147467259) {
                Logger.Warn(ex, "Model download blocked (HResult 0x80004005) — security software may be blocking outbound HTTP.");
                Contingency.NotifyHttpAccessDenied("download the Whisper AI model");
                if (_cornerIcon != null) {
                    _cornerIcon.IconName = "Active Comms: Model error \u2014 check log";
                }
            } catch (Exception ex) {
                ...
                ScreenNotification.ShowNotification(
                    $"Active Comms: Failed to load model \u2014 {ex.GetType().Name}: {ex.Message}",
        ...
        private async Task ProcessAudioAsync(MemoryStream audioStream) {
            try {
                if (_transcriptionService == null) {
                    ScreenNotification.ShowNotification(
                        "Active Comms: Model is still loading, please wait\u2026",
                        ScreenNotification.NotificationType.Warning, null, 5);
                    return;
                }

                Logger.Info($"ActiveComms: audio stream length = {audioStream.Length} bytes");

                string text = await _transcriptionService.TranscribeAudioAsync(
                    audioStream, _customDictionary.Value);

                Logger.Info($"ActiveComms: transcription result = \"{text}\"");

                if (string.IsNullOrWhiteSpace(text)) {
                    ScreenNotification.ShowNotification(
                        "Active Comms: Nothing heard \u2014 speak louder or check your mic.",
                ...
                ScreenNotification.ShowNotification(
                    "Active Comms: Transcription or injection error.",
        ...
        private void StartRecording() {
            ...
            if (_cornerIcon != null) {
                ...
                _cornerIcon.IconName = "Active Comms: Recording\u2026";
            }
            ...
            try {
                ...
                ScreenNotification.ShowNotification(
                    "Active Comms: Failed to start microphone. Is a mic connected?",
                    ScreenNotification.NotificationType.Error, null, 5);
            }
        }
        ...
        private void StopRecordingAndProcess() {
            ...
            if (_cornerIcon != null) {
                _cornerIcon.Icon     = _texIcon;
                _cornerIcon.IconName = "Active Comms: Ready";
            }

            if (_hideMicPopup?.Value != true) {
                ScreenNotification.ShowNotification(
                    "Active Comms: Mic Off",
